...
module PracticalFramework::SharedTestModules::Forms::CreateNewUserWithMembershipInvitationForm::BaseTests
  ...
  included do
    ...
    test "save: merges errors from the attempted new user and re-raises" do
      ...
      form = CreateNewUserWithMembershipInvitationForm.new(
        user: User.new(webauthn_id: SecureRandom.hex),
      ...
      assert_no_difference "User::Passkey.count" do
    ...
    test "save: merges errors when trying to create a user with an existing email" do
      ...
      form = CreateNewUserWithMembershipInvitationForm.new(
        user: User.new(webauthn_id: SecureRandom.hex),
        membership_invitation: membership_invitation,
        email: users.organization_1_department_head.email,
    ...
    test "save: merges errors when trying to create a passkey that already exists" do
      ...
      credential = dummy_webauthn_credential
      credential.public_key = User::Passkey.first.public_key
    ...
    """ do
      ...
      assert_difference "User::Passkey.count", +1 do
        form.save!
      end
      end
      end
