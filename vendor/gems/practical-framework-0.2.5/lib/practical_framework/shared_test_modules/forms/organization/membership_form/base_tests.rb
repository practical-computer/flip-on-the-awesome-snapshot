...
module PracticalFramework::SharedTestModules::Forms::Organization::MembershipForm::BaseTests
  ...
  included do
    test "raises a validation error if trying to archive the only organization_manager" do
      ...
      form = Organization::MembershipForm.new(current_user: user,
    ...
    test "can change the membership_type for a staff member" do
      user = users.organization_1_owner
      organization = organizations.organization_1
      membership = users.moonlighter.memberships.find_by!(organization: organization)
