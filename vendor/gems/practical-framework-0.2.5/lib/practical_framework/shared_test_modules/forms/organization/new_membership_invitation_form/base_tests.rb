...
module PracticalFramework::SharedTestModules::Forms::Organization::NewMembershipInvitationForm::BaseTests
  extend ActiveSupport::Concern
  included do
    test "save!: raises a validation error if the email is blank" do
      form = Organization::NewMembershipInvitationForm.new(email: "    ")
    ...
    test "save!: raises a validation error if the current_organization is missing" do
      form = Organization::NewMembershipInvitationForm.new(current_organization: nil)
      assert_raises ActiveModel::ValidationError do
        form.save!
      end
    ...
    test "save!: raises validation errors from the underlying MembershipInvitation object" do
      form = Organization::NewMembershipInvitationForm.new(email: Faker::Internet.email,
    ...
    test "save!: creates a new MembershipInvitation and enqueues the email" do
      ...
      form = Organization::NewMembershipInvitationForm.new(email: email,
                                                           current_organization: organizations.organization_1,
                                                           sender: users.organization_1_department_head,
      ...
      assert_equal users.organization_1_department_head, form.invitation.sender
    ...
    test "save!: returns an error if the MembershipInvitation cannot be re-sent" do
      ...
      email = user.email
      existing_invitation = organizations.organization_1.membership_invitations.create!(email: email, sender: users.organization_1_department_head, membership_type: :staff, user: user)
