...
module PracticalFramework::SharedTestModules::Controllers::Users::MembershipsController::BaseTests
  ...
  included do
    test "index: lists all non-archived-by_organization membership types and membership invitations" do
      ...
      inivtation_organization = Organization.create(name: Faker::Company.name)
      inivtation_organization.memberships.create!(user: users.buffy, membership_type: :organization_manager, state: :active)
      ...
      membership_invitation = inivtation_organization.membership_invitations.create!(email: user.email, membership_type: :staff, sender: users.buffy)
