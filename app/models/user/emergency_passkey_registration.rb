...
class User::EmergencyPasskeyRegistration < ApplicationRecord
  class AlreadyUsedError < StandardError; end

  belongs_to :user
  belongs_to :passkey, optional: true, foreign_key: :user_passkey_id
  ...
  def use_for_and_notify!(new_passkey:)
    ...
    User::PasskeyMailer.passkey_added(passkey: new_passkey).deliver_later
  end
end
