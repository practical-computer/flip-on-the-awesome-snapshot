...
class User::PasskeyMailerTest < ActionMailer::TestCase
  include PracticalFramework::TestHelpers::PostmarkTemplateHelpers
  include Rails.application.routes.url_helpers

  test "passkey_added" do
    passkey = users.buffy.passkeys.first

    mail = User::PasskeyMailer.passkey_added(passkey: passkey)

    assert_equal "Postmark Template: \"passkey-added\"", mail.subject
    assert_equal [passkey.user.email], mail.to
    ...
    assert_equal passkey.user.name, template_model["name"]
  ...
  test "passkey_removed" do
    ...
    mail = User::PasskeyMailer.passkey_removed(
      user: user,
      passkey_label: label,
    ...
    assert_equal root_url, template_model["product_url"]
    assert_equal AppSettings.app_name, template_model["product_name"]
    assert_equal user.name, template_model["name"]
